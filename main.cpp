...
void processInput(GLFWwindow* window, glm::vec3& playerPosition, glm::vec3& cameraPosition, glm::vec3& cameraFront, glm::vec3& cameraUp);
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        // input
        // -----
        processInput(window, playerPosition, cameraPosition, cameraFront, cameraUp);
...
void processInput(GLFWwindow* window, glm::vec3& playerPosition, glm::vec3& cameraPosition, glm::vec3& cameraFront, glm::vec3& cameraUp)
{
    ...
    const float lastFrameTime = 0.1f;
    ...
    float xOffset = mouseX - SCR_WIDTH / 2.0f;
    float yOffset = mouseY - SCR_HEIGHT / 2.0f;
    // Calculate distance from middle
    float distance = sqrt(xOffset * xOffset + yOffset * yOffset);

    // Calculate max distance possible
    float maxDistance = sqrt((SCR_WIDTH / 2.0f) * (SCR_WIDTH / 2.0f) + (SCR_HEIGHT / 2.0f) * (SCR_HEIGHT / 2.0f));
    // Set mouse sensitivity based on distance from the center of the screen
    float mouseSensitivity = 0.0001f + 0.001f * (distance / maxDistance);
